...
class DataSource(resources: Resources) {
    ...
    fun addFlower(carne: Carne) {
        val currentList = flowersLiveData.value
        if (currentList == null) {
            flowersLiveData.postValue(listOf(carne))
        } else {
            val updatedList = currentList.toMutableList()
            updatedList.add(0, carne)
            flowersLiveData.postValue(updatedList)
        }
    }

    /* Removes flower from liveData and posts value. */
    fun removeFlower(carne: Carne) {
        val currentList = flowersLiveData.value
        if (currentList != null) {
            val updatedList = currentList.toMutableList()
            updatedList.remove(carne)
            flowersLiveData.postValue(updatedList)
        }
    }

    /* Returns flower given an ID. */
    fun getFlowerForId(id: Long): Carne? {
    ...
    fun getFlowerList(): LiveData<List<Carne>> {
